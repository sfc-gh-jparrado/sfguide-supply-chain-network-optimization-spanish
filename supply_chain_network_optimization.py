...
# Función auxiliar para reemplazar makeDict de PuLP
def makeDict(indices, values, default=0):
    """
    Crea un diccionario anidado para almacenar valores de costos/distancias
    Args:
        indices: Lista de listas con los índices [keys1, keys2]
        values: Lista de listas con los valores correspondientes
        default: Valor por defecto si no se encuentra una clave
    Returns:
        dict: Diccionario anidado {key1: {key2: value}}
    """
    result = {}
    try:
        keys1, keys2 = indices
            ...
            for j, key2 in enumerate(keys2):
                try:
                    if i < len(values) and j < len(values[i]):
                        ...
                        val = values[i][j]
                        if val is None:
                            result[key1][key2] = default
                        else:
                            result[key1][key2] = float(val) if isinstance(val, (int, float, str)) else default
                    else:
                        result[key1][key2] = default
        ...
        print(f"Error en makeDict: {str(e)}")
        return {}


# Verificar tipo de conexión a Snowflake
def set_session():
    try:
        # Primero intentar obtener la sesión activa (SiS - Streamlit in Snowflake)
        session = get_active_session()
        st.session_state["streamlit_mode"] = "SiS"
        return session
    except:
        try:
            # Si falla, intentar como Native App
            ...
            # Como último recurso, usar conexión externa (OSS)
            try:
                ...
                st.error("Por favor verifica la configuración de conexión")
                st.stop()
...
# necesario después de cualquier cambio en fábricas debido al pivot
...
# necesario después de cualquier cambio en distribuidores debido al pivot
...
class OptimizationModel:
    ...
    def solve_sample(self):
        ...
        # Crea una lista de tuplas que contiene todas las rutas posibles de transporte
        ...
        # La función objetivo se agrega primero a 'prob'
        prob += (
    ...
    def prepare_data(self, session):
        ...
        for i in range(len(factories)):
            f2d_rates.append(f2d_rate_values[i][1:len(distributors)+1])
        ...
        f2d_mileage = []
        for i in range(len(factories)):
            f2d_mileage.append(f2d_mileage_values[i][1:len(distributors)+1])
        ...
        for i in range(len(distributors)):
            d2c_rates.append(d2c_rate_values[i][1:len(customers)+1])
        ...
        d2c_mileage = []
        for i in range(len(distributors)):
            d2c_mileage.append(d2c_mileage_values[i][1:len(customers)+1])
